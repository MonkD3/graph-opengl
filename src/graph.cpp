...
void Graph::step(){
    ...
    for (size_t i = 0; i < n_vtx-1; i++){
        ...
        for (size_t j = i+1; j < n_vtx; j++){
            ...
            dp[2*i] += vx/dist; dp[2*j] -= vx/dist;
            dp[2*i+1] += vy/dist; dp[2*j+1] -= vy/dist;
        }
    }
    for (size_t i = 0; i < 2*n_vtx; i++) pos[i] += dt*Fr*dp[i];
    ...
    for (size_t i = 0; i < n_vtx; i++){
        for (size_t j = rowstart[i]; j < rowstart[i+1]; j++){
            ...
            dp[2*i] -= neigw*vx/dist; dp[2*neig] += neigw*vx/dist;
            dp[2*i+1] -= neigw*vy/dist; dp[2*neig+1] += neigw*vy/dist;
        }
    }

    for (size_t i = 0; i < 2*n_vtx; i++) pos[i] += dt*Fa*dp[i];
    delete [] dp;
}
